...
def open_masw_window(menu_window,images_path):
    ...
    run_button = CTkButton(
        ...
        width=100,
        height=30,
        command= lambda: masw_module(file_content,directory_entry.get()) 
    )
...
def masw_module(file_content,inputs_path):
    ...
    ventana_progreso, barra_progreso, texto_progreso = create_progress_window("Generando memorias","logo.ico",f"0/{len(list(file_content.keys()))-3}")
...
def modificar_hoja_Vs(libro,key,inputs_path):
    ...
    #Obtiene la hoja
    hoja = libro.sheets["Vs"]

    #Parsea el input_path
    inputs_path = inputs_path.replace("/", "\\")

    hoja.pictures.add(f"{inputs_path}\{key}\Model.png",
                        left = hoja.range("C26").left,
                        top = hoja.range("C26").top,
                        width = '400')
...
def modificar_modulos_elasticos(libro,key):
    ...
    hoja_modulos_elasticos["L29"].value = round(30 / sum(results),0)
