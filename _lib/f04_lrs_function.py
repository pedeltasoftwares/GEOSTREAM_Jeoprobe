...
def open_lrs_window(menu_window,images_path):
   ...
   # Crear una nueva ventana
    ...
    window.title("LRS")
    # Ícono ventana
    window.after(201, lambda: window.iconbitmap(os.path.join(images_path, "lrs.ico")))
    # Resizable
...
def lrs_module(file_content,inputs_path,window):
    ...
    results_path = f'{documents_path}//GEOSTREAM//LRS'
    ...
    ventana_progreso, barra_progreso, texto_progreso = create_progress_window("LRS - Generando memorias","logo.ico",f"0/{len(list(file_content.keys()))-3}")
    ...
    for key in list(file_content.keys()):
        if key != "cliente" and key != "proyecto" and key != "OS":
            ...
            #Copia el template
            os.mkdir(f'{documents_path}//GEOSTREAM//LRS//{key}')
            shutil.copy(f'{templates_path}//template lrs.xlsm', f'{documents_path}///GEOSTREAM//LRS//{key}//template lrs.xlsm')

            #Abre el template
            app = xw.App(visible=False)  
            app.display_alerts = False 
            app.screen_updating = False
            libro = xw.Book(f'{documents_path}//GEOSTREAM//LRS//{key}//template lrs.xlsm', update_links=True)

            #Modifica hoja A1
            modificar_hoja_A1(libro,file_content,key)

            #Modificar hoja fotos: Espectro G01
    ...
    for key in list(file_content.keys()):
        if key != "cliente" and key != "proyecto" and key != "OS":
            merger.append(f'{documents_path}//GEOSTREAM//LRS//{key}//{key}.pdf')

    # Guardar el PDF combinado en un archivo
    merger.write(f'{documents_path}//GEOSTREAM//LRS//combinado.pdf')
    merger.close()
...
def modificar_hoja_fotos(libro,key,inputs_path):
    ...
    #Obtiene la hoja
    hoja = libro.sheets["Espectro G01"]
    ...
    hoja.pictures.add(f"{inputs_path}\{key}\{img_name}",
                    left = hoja.range("B9").left,
                    top = hoja.range("C8").top,
                    width = 600,
                    height = 273)
...
def save_to_pdf(libro, key, documents_path ):
    ...
    #Guarda el pdf
    output_pdf_path = f'{documents_path}//GEOSTREAM//LRS//{key}//{key}.pdf'
